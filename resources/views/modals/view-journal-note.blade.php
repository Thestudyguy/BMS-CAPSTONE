                ...
                @foreach ($journal->notes as $item)
                    <div class="card mb-3" style="
                        ...
                        <div class="text-end text-muted small mt-2">
                           {{$journal->accountantLname}}, {{$journal->accountantFname}} - {{$journal->accountantRole}}
                        <br><em>{{ \Carbon\Carbon::parse($item['created_at'])->format('F d, Y \a\t h:i A') }}</em>
                        </div>
                    </div>
                @endforeach
            </div>
